...
import { makeStubProtocolContext, MOCK_CORE_TRAILERS,type ProtocolContext,TEST_ID_KEY, TEST_PROTOCOL_DEFINITION } from '../../../src/engine/testing.js';
import { makeMockInfra } from '../engine-test-utils.js';

describe('Discovery Git Integration', () => {
  let testDir: string;
  let git: GitClient;
  ...
  it('Coarse Filtering: should correctly filter by author at Git level', async () => {
    const result = await findAtoms(infra, { type: 'global', raw: 'all', resolvedPaths: [] }, { author: 'test@example.com' });
  ...
  it('Coarse Filtering: should correctly filter by scope at Git level', async () => {
    const result = await findAtoms(infra, { type: 'global', raw: 'all', resolvedPaths: [] }, { scope: 'auth' });
    expect(result).toHaveLength(1);
  });

  it('Coarse Filtering: should handle AND logic (all-match) at Git level', async () => {
    const result = await findAtoms(infra, { type: 'global', raw: 'all', resolvedPaths: [] }, { author: 'test@example.com', scope: 'auth' });
    expect(result).toHaveLength(1);
  });

  it('Coarse Filtering: should handle date-based filtering (since/until)', async () => {
    const all = await findAtoms(infra, { type: 'global', raw: 'all', resolvedPaths: [] });
    const midPoint = all[0].date;
    const sinceResult = await findAtoms(infra, { type: 'global', raw: 'all', resolvedPaths: [] }, { since: midPoint.toISOString() });
    expect(sinceResult.length).toBeGreaterThanOrEqual(1);
  });

  it('Coarse Filtering: should handle relative dates (e.g., "1 hour ago")', async () => {
    const result = await findAtoms(infra, { type: 'global', raw: 'all', resolvedPaths: [] }, { since: '1 hour ago' });
    expect(result.length).toBeGreaterThanOrEqual(2);
  });

  it('Coarse Filtering: should handle commit references (e.g., "HEAD~2")', async () => {
    const result = await findAtoms(infra, { type: 'global', raw: 'all', resolvedPaths: [] }, { since: 'HEAD~2' });
    expect(result.length).toBeGreaterThanOrEqual(1);
  });

  it('Coarse Filtering: should handle until filtering with refs (e.g., "HEAD~3")', async () => {
    const result = await findAtoms(infra, { type: 'global', raw: 'all', resolvedPaths: [] }, { until: 'HEAD~3' });
  ...
  it('Coarse Filtering: should handle commit hashes', async () => {
    ...
    const result = await findAtoms(infra, { type: 'global', raw: 'all', resolvedPaths: [] }, { until: hash, maxCommits: 1 });
  ...
  it('Coarse Filtering: should handle garbage date strings gracefully', async () => {
    const result = await findAtoms(infra, { type: 'global', raw: 'all', resolvedPaths: [] }, { since: 'not-a-date' });
