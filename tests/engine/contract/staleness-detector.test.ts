...
import { TEST_ENGINE_CONFIG, makeProtocol, makeMockGitClient, makeAtom as makeUtilsAtom, makeTrailers, TEST_ID_KEY } from '../engine-test-utils.js';

function makeAtom(options: any): Atom {
  return makeUtilsAtom({
    ...options,
    trailers: makeTrailers({
      ...
      [TEST_ID_KEY]: options.id ? [options.id] : undefined
    })
  });
}
    ...
    protocol = makeProtocol({
      ...
      trailers: {
        Confidence: {
          ...
          stale_if: { kind: 'value-equals', value: 'low', signal: STALE_SIGNAL.VALUE_MATCH }
        },
        Directive: {
          description: 'Directives',
          multivalue: true,
          stale_if: { kind: 'date-expired', signal: STALE_SIGNAL.EXPIRED_HINT }
        },
        'Depends-on': {
          description: 'Dependencies',
          multivalue: true,
          validation: 'reference',
          stale_if: { kind: 'reference-superseded', signal: STALE_SIGNAL.ORPHANED_DEP }
        }
      }
    });
  ...
  describe('analyze', () => {
    ...
    describe('protocol-specific signals (delegation)', () => {
      it('should detect atoms with Confidence: low', async () => {
        ...
        expect(result).toHaveLength(1);
        expect(result[0].reasons.some((r) => r.signal === STALE_SIGNAL.VALUE_MATCH)).toBe(true);
      });
    ...
    describe('combined signals', () => {
      it('should report multiple staleness reasons for one atom', async () => {
        ...
        expect(signals).toContain(STALE_SIGNAL.VALUE_MATCH);
