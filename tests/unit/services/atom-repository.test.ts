...
describe('AtomRepository', () => {
  ...
  describe('findByTarget', () => {
    ...
    it('should include dependencies from different authors when using --follow', async () => {
      ...
      vi.mocked(gitClient.resolveRef).mockResolvedValue('head-sha');
  ...
  describe('Integration: AtomCache', () => {
    ...
    it('should use QueryCache and bypass discovery when cache hits', async () => {
      ...
      expect(gitClient.log).not.toHaveBeenCalled();
      expect(queryCache.get).toHaveBeenCalledWith('head-sha', makeGitLogArgs(), makeQueryOptions());
    ...
    it('should update QueryCache on discovery miss', async () => {
      ...
      expect(queryCache.prune).toHaveBeenCalled();
    });

    it('should batch hydration from QueryCache', async () => {
      // Create 60 hashes to trigger 3 batches (batch size is 20)
      const hashes = Array.from({ length: 60 }, (_, i) => `h${i.toString().padStart(7, '0')}`);
      vi.mocked(gitClient.resolveRef).mockResolvedValue('head-sha');
      ...
      expect(gitClient.getCommitsByHashes).toHaveBeenNthCalledWith(1, [hashes[0], hashes[59]]);
      expect(gitClient.getCommitsByHashes).toHaveBeenNthCalledWith(2, hashes.slice(0, 20));
      expect(gitClient.getCommitsByHashes).toHaveBeenNthCalledWith(3, hashes.slice(20, 40));
      expect(gitClient.getCommitsByHashes).toHaveBeenNthCalledWith(4, hashes.slice(40));
      expect(queryCache.prune).toHaveBeenCalled();
    });
