...
describe('QueryCache', () => {
  ...
  it('should reject invalid head hashes', async () => {
    const cache = new QueryCache(cacheDir);
    expect(await cache.get('../../../etc/passwd', gitLogArgs, options)).toBeNull();
    expect(await cache.get('not-a-hash!', gitLogArgs, options)).toBeNull();
  });
