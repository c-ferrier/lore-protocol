...
/** Yet Another Protocol schema (Namespaced, Permissive). */
export const TEST_YAP_DEFINITION: ProtocolDefinition = {
  ...
  identityKey: 'YAP-id',
  trailers: {
    'YAP-id': { description: 'YAP identity.', multivalue: false, validation: 'pattern', pattern: '^[0-9a-f]{8}$', isCore: true },
    'Impact': { 
        description: 'YAP impact.', multivalue: false, validation: 'values',
        values: { low: { description: '' }, high: { description: '' } },
        squash: 'rank-max', isCore: true 
    }
  }
};

/** STRICT: A namespaced, non-permissive protocol definition. */
export const TEST_STRICT_DEFINITION: ProtocolDefinition = {
  name: 'Strict',
  version: '1.0',
  namespace: 'st',
  strict: true,
  ...
  trailers: {
    'Strict-id': { description: 'Strict identity.', multivalue: false, validation: 'pattern', pattern: '^[0-9a-f]{8}$', isCore: true },
    'Required-Key': { description: 'Must exist.', multivalue: false, validation: 'none', required: true, isCore: true }
  }
};
...
export function makeProtocol(
    ...
    // 1. Create a FRESH baseline definition.
    // NON-GREEDY: If trailers are provided, we do NOT merge with the global TEST_PROTOCOL_DEFINITION.
    const baseDef: ProtocolDefinition = {
        name: defOverrides.name ?? 'Mock',
        version: defOverrides.version ?? '1.0',
        namespace: defOverrides.namespace ?? '',
        strict: defOverrides.strict ?? false,
        permissive: defOverrides.permissive ?? true,
        identityKey: defOverrides.identityKey ?? `${defOverrides.name ?? 'Mock'}-id`,
        trailers: { ...defOverrides.trailers },
        ...defOverrides
    };

    // 2. Inject default identity ONLY if trailers object is empty. 
    if (Object.keys(baseDef.trailers).length === 0) {
        baseDef.trailers[baseDef.identityKey] = {
            description: 'id', multivalue: false, validation: 'pattern', pattern: '^[0-9a-f]{8}$',
            required: true, isCore: true
        };
    }
...
export function makeRawCommit(options: { hash?: string; date?: string; author?: string; subject?: string; body?: string; trailers?: string; } = {}): RawCommit {
  return {
